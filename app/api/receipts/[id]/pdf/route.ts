...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: receipt, error } = await sb.from("receipts").select("*").eq("id", params.id).single();
...
<div class="row header">
  <div class="col">
    ...
    <div class="muted">Date: <span class="title">${new Date(r.date || Date.now()).toLocaleDateString()}</span></div>
  </div>
...
<div class="card" style="margin-top:16px">
  <table>
    ...
    <tfoot>
      ...
      ${r.tax ? `<tr class="totals"><td colspan="3" class="right">Tax</td><td class="right">${fmt(r.tax)}</td></tr>` : ""}
